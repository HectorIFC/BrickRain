...
static func line_points(cleared_count: int, level: int) -> int:
	if cleared_count == 1:
		return 100 * level
	if cleared_count == 2:
		return 300 * level
...
static func apply_clear(score_state: Dictionary, cleared_count: int) -> Dictionary:
	var points := line_points(cleared_count, int(score_state["level"]))
